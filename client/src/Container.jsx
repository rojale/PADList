...
export default class Container extends Component {
	constructor(props) {
		...
		const element_filter = elements.map((element, i) => {
			return { name: element, filter: false, key: i };
		});

		const type_filter = types_data.map((type, i) => {
			let typeString = type.replace("/", "");
			return { name: typeString.replace(" ", ""), filter: false, key: i };
		});
	...
	changeElementFilter(filterList) {
		const activeFilters = filterList.map(element => {
	...
	changeASFilter(filterList) {
		const activeFilters = filterList.map(as => {
			return as.value;
		});
	...
	changeTypeFilter(filterList) {
		const activeFilters = filterList.map(type => {
			return type.value;
		});
	...
	render() {
		const asFilter = this.state.as_filter
			.filter(filter => {
		...
		const elementFilter = this.state.element_filter
			.filter(filter => {
				return filter.filter;
			})
			.map(filter => {
		...
		const typeFilter = this.state.type_filter
			.filter(filter => {
				return filter.filter;
			})
			.map(filter => {
						...
						<div style={{ textAlign: "left" }}>
							<div
								style={{
									...
									width: 20,
									margin: 1,
									backgroundColor: "rgba(6, 82, 221,0.5)",
									color: "white",
									textAlign: "center",
									cursor: 'help'
								}}
								title="Click on table headers to sort asc/desc. Type in boxes under header to filter (bit buggy, work in progress.) Use menus for multi-select"
							>
								?
							</div>
						</div>
						<Selector
							items={this.state.element_filter}
							onChange={this.changeElementFilter}
							placeholder="Element"
						/>
						<Selector
							items={this.state.as_filter}
							onChange={this.changeASFilter}
							placeholder="Awoken Skills"
						/>
						<Selector
							items={this.state.type_filter}
